...
const pool = new Pool({
  ...
  ssl: {
    rejectUnauthorized: false // This allows the connection even if the SSL certificate isn't verified
  }
});
...
pool.connect((err) => {
  if (err) {
    ...
    console.log('✅ SUCCESS: Connected to Supabase Cloud Database');
  }
});
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    // Check if account already exists in residents table
...
app.post('/api/announcements', async (req, res) => {
  ...
  try {
    const query = `
      INSERT INTO announcements (title, details, type, scheduled_date, author_name) 
...
// Submit a new request (Resident Side)
app.post('/api/requests', async (req, res) => {
  const { resident_name, resident_email, document_type, purpose } = req.body;
  try {
    const query = `
      INSERT INTO document_requests (resident_name, resident_email, document_type, purpose, status) 
...
// Update Request Status (Admin Side)
...
app.get('/api/requests/:email', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT * FROM document_requests WHERE resident_email = $1 ORDER BY created_at DESC', 
      [email]
    );
    res.json(result.rows);
  } catch (err) {
